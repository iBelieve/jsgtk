...
const
  ...
  fs = {
    existsSync: function existsSync(file) {
      let fd = GFile.new_for_path(file)

      return fd.query_exists(null)
    },
      ...
      GFile.new_for_path(file)
        .load_contents_async(null, (source, result) => {
          ...
          try {
            ...
            callback(null, encoding ? data.toString(encoding).slice(1) : data);
          } catch(err) {
            callback(err);
          }
        });
    ...
    readFileSync: function readFileSync(file, options) {
      ...
      let encoding = typeof options === 'string' ?
        options : (options && options.encoding || '');
      return encoding ? data.toString(encoding).slice(1) : data;
    },
